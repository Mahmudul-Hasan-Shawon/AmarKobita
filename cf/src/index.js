...
import { ok, fail, parseQuery, parseParams } from './util.js';
...
async function getCookieUser(request, env, cookieName) {
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api' || url.pathname.startsWith('/api/')) {
      ...
      const user = await getCookieUser(request, env, 'auth_token');
      const siteUser = await getCookieUser(request, env, 'user_token');
