...
export const routes = [
  {
    ...
    handler: async (ctx) => {
      ...
      try {
        ...
        const totalUsers = (await one(db, 'SELECT COUNT(*) as count FROM users'))?.count ?? 0;
        const pendingSubmissions = (await one(db, "SELECT COUNT(*) as count FROM submissions WHERE status = 'pending'"))?.count ?? 0;
        const approvedSubmissions = (await one(db, "SELECT COUNT(*) as count FROM submissions WHERE status = 'approved'"))?.count ?? 0;
        const rejectedSubmissions = (await one(db, "SELECT COUNT(*) as count FROM submissions WHERE status = 'rejected'"))?.count ?? 0;

        return ok({
          totalWritings, publishedWritings, draftWritings,
          totalAuthors, totalCategories, totalCollections, totalViews, totalSaves,
          totalUsers, pendingSubmissions, approvedSubmissions, rejectedSubmissions,
        });
      } catch {
